...
void Node<T>::print()
{
    cout << " * " << data << ": [addr: " << this << " next: " << this->next << " prev: " << this->prev << endl; //<< " up: " << this->up << " down: " << this->down << "]  ";
}
...
Node<T>* LinkedList<T>::insert(Node<T>* location, T data) {
    ...
    Node<T>* insertedElement = new Node<T>(data);
    if (location->next == nullptr || location->next->data >= data) {
        insertedElement->prev = location;
        insertedElement->next = location->next;


        location->next = insertedElement;
        if (insertedElement->next != nullptr) {
            insertedElement->next->prev = insertedElement;
